"""Re-ingest only the Hermes provider after adding a general overview document.

Re-scrapes and re-chunks the Hermes docs, then:
- Replaces only the "hermes" points in the Qdrant collection.
- Replaces only the "hermes" entries in the BM25 index / docs store,
...
def main() -> None:
    ...
    orchestrator = IngestionOrchestrator(scrapers=[HermesScraper()])
    ...
    # --- Qdrant: drop old hermes points, upsert new ones ---
    client: QdrantClient = orchestrator.qdrant_client
    orchestrator._ensure_collection()
    client.delete(
        collection_name=settings.QDRANT_COLLECTION,
        points_selector=qm.FilterSelector(
            filter=qm.Filter(must=[qm.FieldCondition(key="provider", match=qm.MatchValue(value="hermes"))])
        ),
    )
    orchestrator.upsert_to_qdrant(embedded_points)

    # --- BM25 / docs store: replace hermes entries, keep everything else ---
    with open(settings.DOCS_STORE_PATH, "rb") as file_handle:
        existing_docs = pickle.load(file_handle)
    other_docs = [doc for doc in existing_docs if doc.get("provider") != "hermes"]
